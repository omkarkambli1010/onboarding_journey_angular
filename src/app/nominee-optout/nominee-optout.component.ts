...
@Component({
  selector: 'app-nominee-optout',
  templateUrl: './nominee-optout.component.html',
  styleUrls: ['./nominee-optout.component.css']
})
export class NomineeOptoutComponent  implements OnDestroy, OnInit {
  @Output()
  ...
  getNomineeIdfyURL() {
    ...
    this._http.postRequest('NomineeGetIdfy', reqData).subscribe((resp) => {
      let response: any = resp.body;
      if (response.status == true) {
        response = JSON.parse(
          this.aesService.decrypt(response.data, this.clientid, this.clientid)
  ...
  //To be changed with redirection flow, call Microservice API here.
  redirectToTwo() {
    if (!this.isGeolocationSubscribed) {
      this.PersonalFormTwo = false;
      this.PersonalFormThree = true;
