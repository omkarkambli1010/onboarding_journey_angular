...
@Component({
  selector: 'app-upload-aadhaar-front',
  templateUrl: './upload-aadhaar-front.component.html',
  styleUrls: ['./upload-aadhaar-front.component.css']
})
export class UploadAadhaarFrontComponent {
  ...
  ngOnInit(): void {
    this.title.setTitle('Upload Aadhaar - Onboarding-DIY-PWA');

    this.meta.updateTag({
      name: 'description',
      content: 'Uploading the Aadhaar of the customer.',
    });
  ...
  uploadPanUploadImg() {
    ...
    var reqData = {
      documentType: 'OCR_AddressProof_Front',
      formNumber: window.sessionStorage.getItem('FormNumber'),
      flag: 'docBase64String',
    ...
    this._http
      .postFilerequest('api/v1/manualuplaod/upload-multipart', formData)
      .subscribe((response: any) => {
        ...
        if (response.body.status == true) {
          ...
          this.toastr.warning(
            'Please check the Aadhaar image uploaded by you. Few details are not matching with the Aadhar data entered in the beginning',
            '',
            {
              positionClass: 'toast-bottom-center',
              timeOut: 5000,
  ...
  async fileChangeEvent(event: any): Promise<void> {
    ...
    if (await isAllowed!=true){
      this.toastr.warning('Failed to Upload Aadhaar', 'Warning', {
        positionClass: 'toast-bottom-center',
        timeOut: 3000,
      });
      this.spinner.hide();
