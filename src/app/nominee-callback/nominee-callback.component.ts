...
@Component({
  selector: 'app-nominee-callback',
  templateUrl: './nominee-callback.component.html',
  styleUrls: ['./nominee-callback.component.css']
})
export class NomineeCallbackComponent {
...
{ 
  ...
  ngOnInit(): void {
    ...
    this.title.setTitle('Nominee Opt-out - Onboarding-DIY-PWA');
  ...
  getNomineeWebhookStatus() {
    if ((this.apiExecstart = 1)) {
      ...
      this._http
        .postRequest('getwebhookDetails', reqData)
        .subscribe((resp) => {
          ...
          if (response.status == true) {
            ...
            this.router.navigate(['/esign']);
              ...
              clearInterval(this.aggWebHookInterval);
              this.toastr.error(
                'Oops! We could not fetch your Consent.',
                '',
                {
                  positionClass: 'toast-bottom-center',
                  timeOut: 3000,
